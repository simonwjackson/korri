...
in
{
  # Stable module key so multiple imports (kiosk + server) deduplicate
  ...
  options.services.korri.cli = {
    ...
    package = lib.mkOption {
      ...
      description = ''
        ...
        Auto-enabled by `services.korri.kiosk` and `services.korri.server`
        via `lib.mkDefault`, so client and stream-host appliances always
        ship the CLI. Set `services.korri.cli.enable = false` to opt out
        if the deployment provides `korri` through another channel.
      '';
    };
  };
