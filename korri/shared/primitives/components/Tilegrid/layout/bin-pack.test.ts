...
import { type GridItemShape, paginateItems } from "./bin-pack"
...
describe("paginateItems", () => {
  ...
  it("flows generic item shapes through unchanged", () => {
    ...
    const tiles: Tile[] = [
      { id: "x", span: 1, payload: { color: "red" } },
      { id: "y", span: 1, payload: { color: "blue" } },
    ]
    const result = paginateItems<Tile>({ items: tiles, columns: 2, rows: 1 })
    expect(result.totalPages).toBe(1)
    expect(result.pages[0]?.[0]?.payload.color).toBe("red")
    expect(result.pages[0]?.[1]?.payload.color).toBe("blue")
  })
})
