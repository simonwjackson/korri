...
/**
 * Gamepad spatial navigation E2E demo.
 *
 * Installs a fake Standard-layout Gamepad before Storybook preview starts so
 * the real gamepad adapter polls navigator.getGamepads() exactly as it would
 * in production. This proves device-agnostic input reaches the same DOM focus
 * engine as keyboard input without component coupling.
 */
...
test.describe("spatial navigation: GameGrid story via gamepad", () => {
  ...
  test("confirm button fires a click on the focused card", async ({ page }) => {
    const buttons = page.locator("button[aria-label]")
    await buttons.first().focus()
