/**
 * Pure pagination math for GridView.
 *
 * Given a list of items (some with multi-cell spans) and a grid layout,
 * returns an array of pages where each page is a list of items that fit.
 * Items are placed top-to-bottom, left-to-right; an item that doesn't fit on
 * the current page starts a new page. Spans larger than min(columns, rows)
 * are clamped.
 */
