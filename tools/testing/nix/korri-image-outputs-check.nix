...
let
  ...
  checks = [
    ...
    # RetroArch binary wrapper and ships the first-party mGBA, Genesis Plus GX, Mesen, PCSX ReARMed, PPSSPP, and bsnes
    # libretro cores by default.
    (check "x86 kiosk RetroArch closure must contain exactly six libretro cores" (
      builtins.length (retroarchCoresFor kiosk) == 4
    ))
    (check "x86 kiosk RetroArch closure must contain mGBA, Genesis Plus GX, Mesen, PCSX ReARMed, PPSSPP, and bsnes" (
      let cores = retroarchCoresFor kiosk; in
      hasRetroarchCore "mgba" cores && hasRetroarchCore "genesis-plus-gx" cores && hasRetroarchCore "mesen" cores && hasRetroarchCore "pcsx-rearmed" cores && hasRetroarchCore "ppsspp" cores && hasRetroarchCore "bsnes" cores
    ))
    (check "Product live USB RetroArch closure must contain exactly six libretro cores" (
      builtins.length (retroarchCoresFor liveUsb) == 4
    ))
    (check "Product live USB RetroArch closure must contain mGBA, Genesis Plus GX, Mesen, PCSX ReARMed, PPSSPP, and bsnes" (
      let cores = retroarchCoresFor liveUsb; in
      hasRetroarchCore "mgba" cores && hasRetroarchCore "genesis-plus-gx" cores && hasRetroarchCore "mesen" cores && hasRetroarchCore "pcsx-rearmed" cores && hasRetroarchCore "ppsspp" cores && hasRetroarchCore "bsnes" cores
    ))
    (check "Developer live USB RetroArch closure must contain exactly six libretro cores" (
      builtins.length (retroarchCoresFor liveUsbDeveloper) == 4
    ))
    (check "Developer live USB RetroArch closure must contain mGBA, Genesis Plus GX, Mesen, PCSX ReARMed, PPSSPP, and bsnes" (
