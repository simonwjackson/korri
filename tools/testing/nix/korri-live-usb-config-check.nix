...
let
  ...
  checks = [
    ...
    # first-party mGBA, Genesis Plus GX, Mesen, and bsnes libretro cores.
    (check "live USB compositor PATH must include exactly one retroarch-bare wrapper" (
      builtins.length retroarchWrappers == 1
    ))
    (check "live USB RetroArch closure must contain exactly four libretro cores" (
      builtins.length retroarchCores == 4
    ))
    (check "live USB RetroArch closure must contain mGBA, Genesis Plus GX, Mesen, and bsnes" (
