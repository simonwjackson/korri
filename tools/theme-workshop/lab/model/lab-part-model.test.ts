...
describe("lab part model", () => {
  ...
  it("collapses a dynamic-tag state family to one tree entry and derives its states", () => {
    const index = buildStoryIndex({
      stories: [
        ...
        },
        {
          ...
          name: "Home · Ready",
          state: "Ready",
          variants: ["home-loading", "home-loaderror"],
          render: () => "r",
        },
        {
          id: "home-loaderror",
          ...
          state: "LoadError",
          variants: ["home-loading", "home-ready"],
          render: () => "e",
        },
      ],
    })
    ...
    expect(index.groups[0]?.stories.map(story => story.id)).toEqual([
      "home-ready",
    ])
    ...
    expect(statesForStory(rep, index.byId).map(state => state.id)).toEqual([
      "Ready",
      "Loading",
      "LoadError",
    ])
    expect(statesForStory(rep, index.byId).map(state => state.label)).toEqual([
      "Ready",
      "Loading",
      "Load error",
    ])
    ...
    expect(stateVariantFor(rep, "LoadError", index.byId)?.id).toBe(
      "home-loaderror",
    )
  })
