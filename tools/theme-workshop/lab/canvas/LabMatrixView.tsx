  ...
  return (
    <div className="lab-matrix-wrap">
      <div className="lab-matrix-controls">
        <label>Rows <select value={rows} onChange={event => setRows(event.target.value as Axis)}><AxisOptions /></select></label>
        <label>Columns <select value={cols} onChange={event => setCols(event.target.value as Axis)}><AxisOptions /></select></label>
      </div>
      <div className="lab-matrix" style={{ gridTemplateColumns: `150px repeat(${colVals.length}, minmax(220px, 1fr))` }}>
        <div className="lab-matrix-head" />
        {colVals.map(col => <div key={col.id} className="lab-matrix-head">{col.label}</div>)}
        {rowVals.map(row => (
          <Fragment key={row.id}>
            <div className="lab-matrix-rowhead">{row.label}</div>
            {colVals.map(col => {
              ...
              return (
                <div key={`${row.id}-${col.id}`} className="lab-matrix-cell" style={ratio ? { aspectRatio: String(ratio) } : undefined}>
                  {unsupportedSource ? (
