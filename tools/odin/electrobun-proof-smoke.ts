...
export function evaluateElectrobunProof(
  ...
  if (!input.status) {
    ...
    if (input.status.profile !== "odin") {
      warnings.push(`Electrobun profile is ${input.status.profile}, not odin`)
    }
...
async function main() {
  const statusPath =
    ...
    "/storage/.local/share/nix-apps/korri-electrobun/status.json"
