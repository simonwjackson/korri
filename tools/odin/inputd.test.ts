...
describe("korri inputd", () => {
  ...
  it("keeps global kill shortcuts active while suppressing inactive raw frames", async () => {
    const proc = await loadProcFixture("bus-input-devices-odin.txt")
    ...
    await waitFor(() => actions.includes("kill-current-game"), "kill action")
  ...
  it("keeps session toggle shortcuts active while suppressing inactive raw frames", async () => {
    const proc = await loadProcFixture("bus-input-devices-odin.txt")
    const source = createControllableEventSource()
    const actions: KorriInputdActionId[] = []
    const handle = await startInputd({
  ...
  it("keeps only one pending system action while no client is connected", async () => {
    const proc = await loadProcFixture("bus-input-devices-odin.txt")
    const source = createControllableEventSource()
    const handle = await startInputd({
      readProcDevices: async () => proc,
    ...
    const client = connectClient(handle.port)
    await client.open()
    client.ws.send(JSON.stringify({ classes: ["system"] }))

    await waitFor(
      () =>
        client.messages
          .map(message => decodeNativeInputEvent(message))
          .some(message => message.kind === "action"),
      "pending system action",
    )
    expect(
      client.messages
        .map(message => decodeNativeInputEvent(message))
        .filter(message => message.kind === "action"),
    ).toHaveLength(1)

    client.close()
  })

  it("does not convert inactive raw d-pad input into action frames", async () => {
    const proc = await loadProcFixture("bus-input-devices-odin.txt")
    const source = createControllableEventSource()
    const handle = await startInputd({
      readProcDevices: async () => proc,
    ...
    source.push(evdevEvent(3, ABS_HAT0X, 1))
    await Bun.sleep(30)

    expect(
      client.messages
        .map(message => decodeNativeInputEvent(message))
        .some(message => message.kind === "action" || message.kind === "input"),
    ).toBe(false)
