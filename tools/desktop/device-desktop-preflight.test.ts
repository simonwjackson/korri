...
import {
  classifyDeviceDesktopPreflight,
  parseDeviceDesktopPreflightFacts,
} from "./device-desktop-preflight"
...
const readyFacts = {
  ...
  korriDesktopAppPath: "/nix/store/hash-korri-desktop/bin/korri-desktop-device",
...
describe("Device desktop preflight", () => {
  test("reports ready when the Device has Layer 8 substrate and a Nix app candidate", () => {
    const report = classifyDeviceDesktopPreflight(readyFacts)

    expect(report.ok).toBe(true)
    expect(report.status).toBe("ready")
    expect(report.messages).toContain(
      "Device Layer 8 Electrobun preflight passed.",
    )
  })

  test("reports substrate-ready when the Korri app has not been installed yet", () => {
    const report = classifyDeviceDesktopPreflight({
      ...readyFacts,
  ...
  test("blocks when SSH cannot reach the Device", () => {
    const report = classifyDeviceDesktopPreflight({
      ...readyFacts,
      sshReachable: false,
      architecture: null,
    ...
    expect(report.messages).toContain("Device is not reachable over SSH.")
    expect(report.recommendations.join("\n")).toContain("DEVICE_HOST")
  })
  ...
  test("blocks when the selected Korri app is not Nix-managed", () => {
    const report = classifyDeviceDesktopPreflight({
      ...readyFacts,
      korriDesktopAppPath: "/usr/bin/korri-desktop-device",
  ...
  test("parses shell-collected key value facts", () => {
    const facts = parseDeviceDesktopPreflightFacts(
      [
        "ssh_reachable=yes",
        "architecture=aarch64",
        "project_exists=yes",
        ...
        "korri_desktop_app_path=/nix/store/hash/bin/korri-desktop-device",
    ...
    expect(facts).toEqual({
      ...
      nixProfileExists: true,
      portableNixExists: false,
      korriDesktopAppPath: "/nix/store/hash/bin/korri-desktop-device",
