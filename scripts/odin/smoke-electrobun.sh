#!/usr/bin/env bash
# Opt-in proof smoke for the Korri Electrobun Layer 8 renderer on the Odin.

set -euo pipefail

ODIN_HOST="${ODIN_HOST:-root@thor}"
ODIN_PROJECT="${ODIN_PROJECT:-/storage/korri}"
KORRI_ELECTROBUN_APP="${KORRI_ELECTROBUN_APP:-korri-desktop-odin}"
...
KORRI_ELECTROBUN_LOG="${KORRI_ELECTROBUN_LOG:-/storage/korri-electrobun-layer8.log}"
...
log "Launching Electrobun proof candidate on $ODIN_HOST..."
ssh -o ConnectTimeout=5 -o BatchMode=yes "$ODIN_HOST" \
  "cd '$ODIN_PROJECT' && KORRI_ELECTROBUN_APP='$KORRI_ELECTROBUN_APP' KORRI_ELECTROBUN_ROOT='$KORRI_ELECTROBUN_ROOT' KORRI_DESKTOP_STATUS_FILE='$KORRI_ELECTROBUN_STATUS_FILE' KORRI_ELECTROBUN_LOG='$KORRI_ELECTROBUN_LOG' KORRI_ELECTROBUN_GPU_EVIDENCE='$KORRI_ELECTROBUN_GPU_EVIDENCE' bash -s" <<'REMOTE' \
...
export KORRI_DESKTOP_PROFILE=odin
...
KORRI_ELECTROBUN_GPU_EVIDENCE="$KORRI_ELECTROBUN_GPU_EVIDENCE" \
  /storage/bin/bun run tools/odin/electrobun-proof-smoke.ts
REMOTE
