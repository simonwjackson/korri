#!/usr/bin/env bash
# Install/manage the Korri input daemon service on the Odin.
#
...
PORT="${KORRI_INPUT_BRIDGE_PORT:-${ODIN_INPUT_BRIDGE_PORT:-3002}}"
LOG="${KORRI_INPUTD_LOG:-/storage/korri-inputd.log}"
...
ExecStart=$PROJECT/scripts/odin/run-inputd.sh
Restart=always
RestartSec=1
...
install_service() {
  ...
  [ -x "$PROJECT/scripts/odin/run-inputd.sh" ] || fail "missing $PROJECT/scripts/odin/run-inputd.sh"
