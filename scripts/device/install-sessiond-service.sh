#!/usr/bin/env bash
# Install/manage the Korri renderer session supervisor service on the Device.

set -euo pipefail

PROJECT="${DEVICE_APP_ROOT:-/storage/.guest/korri/app}"
PORT="${KORRI_SESSIOND_PORT:-3003}"
LOG="${KORRI_SESSIOND_LOG:-/storage/.guest/korri/logs/sessiond.log}"
TOKEN_FILE="${KORRI_SESSIOND_TOKEN_FILE:-/storage/.guest/korri/sessiond.token}"
ELECTROBUN_APP="${KORRI_ELECTROBUN_APP:-/storage/.nix-profile/bin/korri-desktop-device}"
...
ExecStart=$PROJECT/scripts/device/run-sessiond.sh
Restart=always
RestartSec=1
...
install_service() {
  ...
  [ -x "$PROJECT/scripts/device/run-sessiond.sh" ] || fail "missing $PROJECT/scripts/device/run-sessiond.sh"
  mkdir -p "$(dirname "$LOG")" "$(dirname "$TOKEN_FILE")"
