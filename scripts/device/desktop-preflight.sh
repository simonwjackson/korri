#!/usr/bin/env bash
# Check whether the Device can host the Korri Electrobun Layer 8 renderer path.
...
facts="$(
  if ssh_device "DEVICE_APP_ROOT='$DEVICE_APP_ROOT' KORRI_ELECTROBUN_APP='$KORRI_ELECTROBUN_APP' KORRI_ELECTROBUN_STATE_ROOT='$KORRI_ELECTROBUN_STATE_ROOT' bash -s" <<'REMOTE'
set -euo pipefail
...
resolve_app() {
  ...
  command -v korri-desktop-device 2>/dev/null || command -v korri-desktop 2>/dev/null || true
}
...
printf 'project_exists=%s\n' "$(yes_no_path "$DEVICE_APP_ROOT")"
printf 'env_exists=%s\n' "$(yes_no_path "$DEVICE_APP_ROOT/.env")"
...
printf '%s\n' "$facts" | bun run "$REPO_ROOT/tools/desktop/device-desktop-preflight.ts"
