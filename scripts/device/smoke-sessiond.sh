#!/usr/bin/env bash
# Smoke test the supervised Korri renderer session daemon on the Device.

set -euo pipefail

DEVICE_HOST="${DEVICE_HOST:-root@sm8550}"
DEVICE_APP_ROOT="${DEVICE_APP_ROOT:-/storage/.guest/korri/app}"
KORRI_SESSIOND_URL="${KORRI_SESSIOND_URL:-http://127.0.0.1:3003}"

log()  { printf '\033[0;36m[sessiond-smoke]\033[0m %s\n' "$*"; }
fail() { printf '\033[0;31m[sessiond-smoke]\033[0m %s\n' "$*" >&2; exit 1; }

log "Checking korri-sessiond on $DEVICE_HOST..."
ssh -o ConnectTimeout=5 -o BatchMode=yes "$DEVICE_HOST" \
  "cd '$DEVICE_APP_ROOT' && KORRI_SESSIOND_URL='$KORRI_SESSIOND_URL' /storage/bin/bun run tools/device/sessiond-smoke.ts" \
  || fail "supervised Korri renderer session smoke failed"

log "sessiond smoke passed"
