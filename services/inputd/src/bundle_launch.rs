...
const DEFAULT_ACTIVE_BUNDLE: &str = "/var/lib/korri-bundle/active";
...
fn launch(arguments: Vec<OsString>) -> Result<ExitCode, String> {
    ...
    let executable = resolve_component(&selector, component, Path::new(STORE_ROOT))?;
...
fn is_store_item_root(path: &Path, store_root: &Path) -> bool {
...
fn is_inside_store_item(path: &Path, store_root: &Path) -> bool {
    let Ok(relative) = path.strip_prefix(store_root) else {
        return false;
    };
    let mut components = relative.components();
...
mod tests {
    ...
    fn fixture() -> (tempfile::TempDir, PathBuf, PathBuf) {
        ...
        let package = store.join("inputd-package");
        std::fs::create_dir_all(bundle.join("bin")).unwrap();
        std::fs::create_dir_all(package.join("bin")).unwrap();
        let executable = package.join("bin/korri-inputd");
        std::fs::write(&executable, b"fixture").unwrap();
        let mut permissions = std::fs::metadata(&executable).unwrap().permissions();
        permissions.set_mode(0o555);
        std::fs::set_permissions(&executable, permissions).unwrap();
        symlink(&executable, bundle.join("bin/korri-inputd")).unwrap();
