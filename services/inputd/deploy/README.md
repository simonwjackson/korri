...
- Candidate activation stops all three old user units and proves that they are inactive. It then tightens the Sunshine state modes. The gate switches to the candidate and reloads the user manager. It disables the retained user-unit files and proves that they stay inactive. It restarts the gameplay user manager. It then starts `korri-compositor.service`, `korrid.service`, and `sunshine.service`. The gate verifies their declared process credentials.
...
- Rollback stops the three candidate system services and restores the exact prior generation. It restarts the user manager with the rollback groups. It then restores all six old user-unit state predicates. Enablement restoration is independent. Active rollback units start in this order: X11 headless, Sunshine, then korrid. A failed active-game check leaves the generation and game untouched. Manual inspection is then necessary.
...
For korrid, the compositor, Sunshine, and inputd, the gate requires an explicit unit user and a Nix-store unit file. It requires one live main PID with matching UID and GID values. The gate checks process groups for these services, the gameplay user manager, and each live game unit. It rejects `input` and `uinput` access everywhere. Only inputd can use `korri-control`. Only Sunshine can use `korri-sunshine-uinput`. Required `video` and `render` groups remain allowed. Each game unit hides the compositor control directory and gameplay runtime directory. It receives a read-only bind for Xwayland socket `X0` only. The gate rejects all other visible X11 sockets. Each game unit also uses a private PID namespace, so procfs cannot reveal Sway's root directory.
