...
- Every SSH command uses a shell-escaped argv plus a local transport deadline and an appropriate remote command deadline. The default five-minute remote mutation bound covers one measured NixOS transition plus a user-manager credential restart; the longer local bound lets the remote timeout release the root lock before cleanup. Tests override these bounds to prove timeout rollback. Candidate, rollback, user, and helper paths never enter unquoted remote shell text.
...
- Candidate activation first stops all three old user units and proves they are inactive. It then switches to the candidate, reloads the user manager so the removed immutable global unit links disappear, disables the retained user-unit files, and proves they remain inactive and disabled. It then restarts the gameplay user's systemd manager, starts the system `x11-headless.service`, `korrid.service`, and `sunshine.service` replacements, and verifies their declared process credentials.
