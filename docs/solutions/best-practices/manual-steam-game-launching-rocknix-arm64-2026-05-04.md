...
> Cross-arch update (2026-05-26): the same core shape — Steam Runtime sniper -> Proton -> `Game.exe`, optionally inside nested Gamescope — is now verified on x86/AKA as well. The ARM64-specific pieces in this note still matter on ROCKNIX (FEX/Box64, `steamrtarm64`, the ARM64 client manifest, `/host/lib`, freedreno), but they are not part of the x86 path. For the x86-specific compositor and readiness fixes, see `docs/solutions/runtime-errors/steam-manual-launch-x86-eager-xwayland-dbus-readiness-2026-05-26.md`.
...
> **Scope note (2026-05-27).** This matrix was captured for the *manual Steam GamepadUI* launch path. On 2026-05-27 the in-tree `composeGamescopeLaunchSpec` path (driven by cascade `GamescopePolicy.backend`) was verified to work with `--backend wayland` on the same hardware class — the matrix below is not a general-purpose contraindication. See [runtime-errors/gamescope-backend-auto-fights-sway-for-drm-master-2026-05-27](../runtime-errors/gamescope-backend-auto-fights-sway-for-drm-master-2026-05-27.md) for the in-tree result and the cascade-policy seam that makes `--backend wayland` the kiosk default.

Observed backend behavior (manual Steam GamepadUI launch path, 2026-05-04):

| Backend | Result |
|---|---|
| `--backend wayland` | Aborted during Gamescope input/window setup in earlier tests. (Different invocation path — see scope note above.) |
...
For launch success, prefer Steam's own app-state log over process names. On x86/AKA, host process names were misleading because pressure-vessel and Gamescope wrappers could survive after the game exited; `content_log.txt`'s `App Running` transition was the durable signal.
