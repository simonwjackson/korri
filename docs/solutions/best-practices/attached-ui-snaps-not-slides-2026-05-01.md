...
- Caption snap implementation in the graduated Shift theme: `korri/shared/themes/shift/molecules/ShiftHomeCaption.tsx` (the inline-transform snap) and `korri/shared/themes/shift/templates/ShiftHomeRoot.tsx` (the focus / scroll / resize anchor recompute).
- Original exploration where the learning came from: `korri/shared/design-system/explorations/home-screens/HomeSunlit.stories.tsx` (deleted in the Shift atomic-decomposition graduation; preserved in git history if the in-line code worked example below needs to be cross-referenced).
