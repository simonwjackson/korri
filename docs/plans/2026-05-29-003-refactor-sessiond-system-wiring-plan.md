...
- No work on running sessiond as a non-root user — tracked separately as `backlog/task-004`.
...
- Migrating sessiond off root (`backlog/task-004`): once that lands, the shared-group story introduced here may collapse into a simpler "everyone is the same non-root user" story, or stay as-is. Out of scope here.
...
- Related backlog: `backlog/task-004 - stop-running-as-root.md` (deferred follow-up)
