...
status: active
date: 2026-05-25
origin: docs/brainstorms/2026-05-25-001-sm8550-guest-owned-input-boundary-requirements.md
...
Moonlight Embedded on Sobo is launched with `-input /dev/input/event11`, which restricts evdev auto-discovery to the virtual gamepad and starves the touchscreen (`event4`) of any chance to reach Sunshine. Today's hide of the raw AYN gamepad (`event3`) is incomplete: the host runs InputPlumber with `HIDE_DEVICES_FROM_ROOT=1` (so `MoveSourceDevice` *should* work), the guest also runs InputPlumber and fails its hide with `EROFS` because `/run/udev/rules.d/` is read-only inside the guest, and the guest has no `systemd-udevd` to apply rules even if writable. The brainstorm chose Option B (finish moving input ownership into the guest) with the B1' migration shape: clean break in behavior with a one-release SSH-driven revert path, then decommission. See origin: `docs/brainstorms/2026-05-25-001-sm8550-guest-owned-input-boundary-requirements.md`.
...
- **Origin document:** [docs/brainstorms/2026-05-25-001-sm8550-guest-owned-input-boundary-requirements.md](docs/brainstorms/2026-05-25-001-sm8550-guest-owned-input-boundary-requirements.md)
