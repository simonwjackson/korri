...
- [ ] **Unit 3: Dev API server with allowlisted file I/O and regenerate**
...
- [ ] **Unit 4: Feature-map data hooks and inspector (read-only)**
...
- [ ] **Unit 5: Graph visualization with React Flow + dagre**
...
- [ ] **Unit 6: Raw Markdown editor and save flow**
...
- **API surface parity:** The dev API is internal and undocumented as an external contract. It must not be exposed beyond `localhost`; the server binds to `127.0.0.1` by default. No CORS allowance to other origins.
...
| Path-safety bug allows writes outside allowlist. | Low | High | Pure-module path resolver with explicit allow/deny tests (Unit 3). All routes go through the helper; no inline path math. Server binds to localhost only. |
