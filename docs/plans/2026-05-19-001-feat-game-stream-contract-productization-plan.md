...
The original headless stream proof established the need for a script-triggered streamed game session (see origin: `docs/brainstorms/2026-05-18-headless-game-stream-orchestration-requirements.md`). Live validation has since narrowed the reusable product boundary: Sunshine should expose one stable app, `Korri Stream`, while a trusted local actor writes a launch intent that the foreground runner completes after a successful session launch. Recoverable startup failures may requeue; malformed or stale intents are quarantined.
...
- Modify: `docs/plans/2026-05-18-001-feat-headless-game-stream-runner-plan.md` only if a narrow note is needed to point readers at this follow-up plan.
...
- Related plan: [docs/plans/2026-05-18-001-feat-headless-game-stream-runner-plan.md](2026-05-18-001-feat-headless-game-stream-runner-plan.md)
