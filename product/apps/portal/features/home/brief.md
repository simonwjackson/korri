...
- The home does not own launch behavior. It composes the resume
  shared `useLibraryLaunchController` controller (`product/platform/library/use-library-launch-controller.ts`)
...
This brief overlaps with the Safe Game Resume brief (`product/apps/portal/features/resume/brief.md`) on the resume-target portion. Where they overlap, this brief points back to SGR; where the home adds promises beyond resume (caption tracking, HUD discoverability), the home owns those.
