(() => {
  ...
  window.__YFSGetSetting = function yfsGetSetting(dictionary, key, defaultValue) {
    if (Object.prototype.hasOwnProperty.call(launchSettings, key)) return launchSettings[key];
    if (defaultValue === undefined) return dictionary.ExpObject(key);
    return dictionary.ExpObject(key, defaultValue);
  };

  const hasLaunchPayload = ["sample", "code_url", "code_b64", "code"].some(hasParam) || sessionStorage.getItem("yfsDirectLaunchCode");
  ...
  HTMLCanvasElement.prototype.getContext = function patchedGetContext(type, options) {
    ...
    if (contextType === "webgl" || contextType === "webgl2" || contextType === "experimental-webgl") {
      return originalGetContext.call(this, type, { ...(options || {}), preserveDrawingBuffer: true });
    }
