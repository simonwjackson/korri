(() => {
  ...
  debug.style.cssText = "position:fixed;left:8px;bottom:8px;z-index:2147483647;background:rgba(0,0,0,.78);color:#fff;font:12px/1.35 monospace;padding:8px;border-radius:6px;max-width:46vw;pointer-events:none;white-space:pre-wrap;display:none";
  ...
  const setOverlay = (visible) => {
    if (visible && lastBootFrame) overlay.style.backgroundImage = `url(${lastBootFrame})`;
  ...
  const readLaunchCode = async () => {
    ...
    if (params.has("code_url")) {
      ...
      const response = await fetch(params.get("code_url"), { cache: "no-store" });
      if (!response.ok) throw new Error(`Unable to fetch code_url: ${response.status}`);
      return await response.text();
    }

    if (params.has("code_b64")) {
      state.transport = location.hash.includes("code_b64") ? "hash:code_b64" : "query:code_b64";
      return decodeBase64Url(params.get("code_b64"));
    }

    if (params.has("code")) {
      state.transport = location.hash.includes("code=") ? "hash:code" : "query:code";
  ...
  const codeInputs = () => {
    ...
    return candidates
      ...
      .filter(({ rect, style }) => rect.width > 40 && rect.height > 10 && style.visibility !== "hidden" && style.display !== "none")
  ...
  const dispatchPointer = (target, type, clientX, clientY) => {
    const common = { bubbles: true, cancelable: true, clientX, clientY, button: 0, buttons: type.endsWith("down") ? 1 : 0 };
  ...
  const launch = async () => {
    if (params.has("sample") || params.has("code_url") || params.has("code_b64") || params.has("code") || sessionStorage.getItem("yfsDirectLaunchCode")) {
