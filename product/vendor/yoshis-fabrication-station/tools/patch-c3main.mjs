...
for (const key of keys) {
  const before = source;
  const pattern = new RegExp(`\\b([A-Za-z_$][\\w$]*)\\.ExpObject\\("${key}"(,\\s*([^\\)]+))?\\)`, "g");
  source = source.replace(pattern, (_match, receiver, _defaultGroup, defaultExpr) => {
    if (defaultExpr !== undefined) return `globalThis.__YFSGetSetting(${receiver}, "${key}", ${defaultExpr})`;
...
for (const [key, count] of replacements) console.error(`patch-c3main: ${key} -> ${count}`);
