...
export const EphemeralOverride = Schema.Struct({
  ...
  // Inlined inheritable whitelist. Moonlight overrides intentionally use a
  // narrower schema than persisted readable policy: app.library.launch is
  // unauthenticated on trusted-LAN deployments, so runtime overrides must not
  // expose command/env/raw-argv/key storage/root-peer process surfaces.
  gamescope: InheritableLayer.fields.gamescope,
  moonlight: Schema.optional(MoonlightOverridePolicy),
  env: InheritableLayer.fields.env,
  cwd: InheritableLayer.fields.cwd,
