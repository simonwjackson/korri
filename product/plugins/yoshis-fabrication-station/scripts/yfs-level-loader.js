;(() => {
  ...
  const readLaunchCode = async () => {
    if (!params.has("code_url")) return null
  ...
  const launch = async () => {
    ...
    const gameplayReady = await waitForGameplayToReplaceLoadUi()
    if (!gameplayReady) throw new Error("Gameplay did not replace the load UI")
  ...
  launch().catch(error => {
    ...
    console.error("[YFS direct launch]", error)
  })
})()
