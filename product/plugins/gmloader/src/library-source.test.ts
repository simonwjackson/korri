...
describe("GMLoader installed library source", () => {
  it("adds installed manifests as playable entries and launch specs", async () => {
    ...
    const entries = await Effect.runPromise(
      source.listPlayableEntries?.() ?? Effect.succeed([]),
    )
    const resolved = await Effect.runPromise(
      source.resolveLaunchForGame("@korri:gmloader/sample"),
    )
    ...
    expect(resolved.spec.args).toEqual([
      "-c",
      join(installRoot, "games", "sample", "gmloader.json"),
    ])
  })
  ...
  it("marks installed entries unavailable when no runtime command is configured", async () => {
    ...
    const canResolve = await Effect.runPromise(
      source.canResolveLaunchForGame?.("@korri:gmloader/sample") ??
        Effect.succeed(true),
    )

    expect(
      entries.find(entry => entry.id === "@korri:gmloader/sample")?.launchable,
    ).toBe(false)
  ...
  it("delegates unknown games to the base source", async () => {
    ...
    const resolved = await Effect.runPromise(
      source.resolveLaunchForGame("base"),
    )
...
function baseSource(): LibrarySourceService {
  return {
    ...
    launchSpecFor: id =>
      Effect.succeed(
        id === "base" ? { command: "base-command", args: [] } : undefined,
      ),
