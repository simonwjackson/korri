...
"$PYTHON_BIN" ./uinput-leak-probe.py --json --strip-acl --target-user nobody --disable-in-sway | tee ./last-dedicated-wrapper-result.json

if grep -q '"korriUiIsolatedCandidate": true' ./last-dedicated-wrapper-result.json; then
  echo "RESULT: PASS - dedicated launch-user candidate isolated synthetic devices from Sway/Korri UI"
  exit 0
fi

echo "RESULT: FAIL - dedicated launch-user candidate was not isolated"
exit 1
