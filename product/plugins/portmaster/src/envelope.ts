...
export async function preparePortMasterLaunchEnvelope(
  ...
  const deviceInfoPath = join(controlRoot, "device_info.txt")
  ...
  const deviceInfo = deviceInfoText()
  ...
  await writeFile(deviceInfoPath, deviceInfo)
  await writeFile(join(romsControlRoot, "device_info.txt"), deviceInfo)
...
function deviceInfoText(): string {
  return `# Generated by @korri:portmaster.\nDEVICE_NAME="\${DEVICE_NAME:-korri}"\nDISPLAY_WIDTH="\${DISPLAY_WIDTH:-1280}"\nDISPLAY_HEIGHT="\${DISPLAY_HEIGHT:-720}"\nANALOG_STICKS="\${ANALOG_STICKS:-2}"\nCFW_NAME="\${CFW_NAME:-korri}"\nPM_CAN_MOUNT="\${PM_CAN_MOUNT:-N}"\nexport DEVICE_NAME DISPLAY_WIDTH DISPLAY_HEIGHT ANALOG_STICKS CFW_NAME PM_CAN_MOUNT\n`
}
