...
export function composeBox64LaunchSpec(
  ...
  const gameLibraryPath =
    normalized.gameLibraryPath ?? gameLibraryPathFromCwd(spec.cwd)
  ...
  setIfPresent(
    env,
    "LD_LIBRARY_PATH",
    mergePath(normalized.nativeLibraryPath, spec.env?.LD_LIBRARY_PATH),
  )
