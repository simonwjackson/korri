...
export async function collectSteamInstallSnapshot(
  ...
  if (!manifest) {
    return {
      ...
      message: input.requested
        ? "Install requested; waiting for Steam manifest"
        : "Steam app is not installed",
      providerEvidence: {},
  ...
  try {
    ...
    const buildId =
      stringField(appState, "buildid") ?? stringField(appState, "BuildID")
    ...
    return {
      ...
      nextActionHint:
        state === "installed" ? "none" : state === "failed" ? "retry" : "wait",
    }
  } catch (error) {
    return {
      appId: input.appId,
...
function stateFromManifest(input: {
  ...
  if (input.stateFlags !== undefined && (input.stateFlags & 1024) !== 0)
    return "downloading"
  if (input.stateFlags !== undefined && input.stateFlags !== 0)
    return "installing"
  return "unknown"
}
...
function messageForState(state: PluginInstallState): string {
  switch (state) {
    case "installed":
      return "Steam app is installed"
    case "downloading":
      return "Steam app is downloading"
    case "installing":
      return "Steam app is installing"
    case "requested":
      return "Steam app install requested"
    case "not-installed":
      return "Steam app is not installed"
    case "failed":
      return "Steam app install failed"
    default:
      return "Steam app install state is unknown"
  }
}
