...
- The eleven-patch NVENC candidate adds H.264 live bitrate/FPS/resolution and restore support, exact FFmpeg/NVENC layout proof, one-attempt-per-500-ms limiting, failed/slow-call session withdrawal, and reset-before-publication across every successful encoder creation/replacement path. Android/keyboard range gestures now use exact opaque source/gesture release identities, and Artemis's legacy Activity-exit chord is removed. The authoritative Sunshine, Linux-host, inputd, portal, Shift, and Android automated gates pass for patch `0016` SHA-256 `686decb81379741e01e0b9b0e9105bbe23765a1bf728565767604383983a7074` and ordered digest `68c6f746306b5361dc9c9ccb0e9d3fd034467b24733e1e0cca348ced4fd40914`.
- V30 reached automated candidate acceptance without reboot and preserved the user manager, pairing, private-state digest, exact controller, and eleven-patch provenance. Its first stream exposed that the explicit `encoder=nvenc` service lacked `/run/opengl-driver/lib`, so Sunshine could not load `libcuda.so.1` and fell back to VAAPI. No NVENC acceptance was claimed. The attempt was inspected, the exact test scope was stopped, raw udev mode was restored, rollback predicates matched, and the ledger was reconciled and archived as superseded. The Linux-host fix now exposes the NVIDIA runtime only for explicit NVENC, skips start without a restart loop unless both driver libraries are readable, and makes later explicit-backend failures reject streaming instead of falling back to VAAPI.
