...
- The device gate now resolves the exact unit-declared `sunshine-korri` executable to its canonical in-package target, requires the running process to match it, verifies the independently approved patch-set digest and protected private-tree digest, and rechecks unchanged Sunshine provenance after the HITL restart.
...
- `nix run .#inputd-check`, `nix run .#korrid-check`, Android JVM/APK/native checks, portal/Shift checks, and all Sunshine package/protocol checks pass at Korri commit `b3b6c38078601bbe8f50a14092b5dd44f37571ec`.
- Mountainous branch `unified` is clean at `caf17b39fbd8a704131134e5b75ad1d061f07515`. It imports only `nixosModules.korri-linux-host` and pins the exact local Korri revision above. Nothing is pushed and no PR exists.
- The current candidate is `/nix/store/861q2knsk2whr2nhc0858bhj20sd0bzc-nixos-system-zao-26.05.20260313.c06b4ae`. Its gate digest is `a795f0b387343355e21441621717d4f69e95a1c2f8bb272cad61899f930873ec`. It is rooted on Zao at `/nix/var/nix/gcroots/korri-candidate-unified-caf17b3`.
...
1. Verify the Korri worktree is clean at the final documentation commit above code revision `b3b6c38078601bbe8f50a14092b5dd44f37571ec`. Verify Mountainous is clean at `caf17b39fbd8a704131134e5b75ad1d061f07515` and still pins that exact local Korri revision.
...
4. Use candidate `/nix/store/861q2knsk2whr2nhc0858bhj20sd0bzc-nixos-system-zao-26.05.20260313.c06b4ae`. Use confirmation `CONFIRM-6f1dbc7712c7a65e`; do not reuse any older candidate token.
