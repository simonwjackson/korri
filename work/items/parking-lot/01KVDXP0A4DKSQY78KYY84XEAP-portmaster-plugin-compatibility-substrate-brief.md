...
- Added `presentation.mode = "gamescope"` so compatibility profiles can launch PortMaster scripts through a fulfilled `gamescope-korri` executable. The generated launcher starts gamescope, runs the child port with `WAYLAND_DISPLAY=gamescope-0`, clears `DISPLAY`, keeps `SDL_VIDEODRIVER=wayland`, and focuses `[app_id="gamescope"]`. Bandai validation launched TurboTableTennis through the generated profile-owned launcher and rendered it via the packaged FRT runtime.
