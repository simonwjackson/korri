...
2026-06-18 Phase 8 implemented the plugin-owned armhf/qemu lane: install-time armhf executable wrappers preserve originals under `.korri-qemu-arm/`, launch env prefers `DEVICE_ARCH=armhf`, and qemu/rootfs/library env is recorded in manifests and envelopes. Bandai verified qemu-arm can execute real PortMaster armhf helper binaries (`xdelta3.armhf`, `7zzs.armhf`) after copying Nix-built qemu and armhf glibc/gcc/zlib stores. A forced-armhf SkiFree launch reached qemu execution through the generated wrapper but failed at missing `libSDL2-2.0.so.0`; game-rendering validation needs a plugin-owned armhf runtime-lib pack next.
