...
This plan extends, and does not replace, the work landed in `../01KQDTYV05D4Q8T63WBQSHD6A1-feat-electrobun-desktop-wrapper/plan.md`. The product behavior of the desktop app — same-origin loopback HTTP, reused `honoApp`, SPA fallback, traversal protection — is unchanged.

## Problem Frame

The Electrobun desktop wrapper landed previously, but its NixOS story is currently a guardrail: `tools/desktop/electrobun-runtime-check.ts` detects the dynamic-linker stub and refuses to proceed (see origin: `./requirements.md`). Local desktop development on the project's `nix develop` shell is broken, and there is no way to launch the desktop app from a remote flake.
...
Stable IDs carried from `./requirements.md` (R1–R14).
...
- Prior plan (extended, not replaced): `../01KQDTYV05D4Q8T63WBQSHD6A1-feat-electrobun-desktop-wrapper/plan.md`
