...
The desktop bun forwards `/api/*` to one configured upstream (`lastConnectedServer.controlUrl`), which means a device running its own korri-server cannot see its own library while connected to a remote. The origin document captures the situation, the federation thesis, and the removal scope; this plan defines how that thesis lands in code. See origin: `./requirements.md`.

---

## Requirements

All R-IDs trace to origin (`./requirements.md`). Carrying forward:
...
All non-goals carry forward verbatim from origin (`./requirements.md`). Summary:
...
- **Prior brainstorms (referenced from origin):**
  - `../../01KRYRGG17Y6WHJ5F91BZTTYC6-feat-korri-lan-stream-discovery/requirements.md` (mDNS advertise/browse infrastructure)
  - `../../01KS1AX718CW379VHCYVJXEW82-feat-headless-source-aware-server/requirements.md` (source-aware federation thesis)
- **Institutional learnings:**
