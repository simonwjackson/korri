...
> Migrated 2026-06-10 from nix-on-rocks backlog task-028: the work is owned by
> this repo (gamescope-korri derivation, launcher launch spec, library config);
> only the PanVK session environment touches substrate guest modules.
...
- [ ] Whichever path is chosen (gamescope patch vs libmali swap) is shipped as a committed Nix derivation change — not as a /run/ drop-in.
- [ ] The chosen path (patched gamescope vs libmali stack) is documented with the reasoning so a future maintainer can revisit when upstream PanVK matures.

## Related

- `../nix-on-rocks/guest/modules/rk3566.nix`
- `../nix-on-rocks/guest/profiles/devices/rg353m.nix`
- `../nix-on-rocks/docs/plans/2026-06-04-001-feat-rg353m-rk3566-support-plan.md`
- `../nix-on-rocks/backlog/task-021 - bring-up-rg353m-display-touchscreen-and-controls.md`
- `../nix-on-rocks/backlog/task-013 - bring-up-rk3566-rk817-audio-in-guest.md`
- `backlog/task-012 - fix-korri-server-app-library-launch-bigint-parse-defect.md`
...
**Payload re-render + fast (guest-promote).** The gamescope-korri patches live in this repo (committed 2804f46, 263f554) → they reach the device via the product payload (`../nix-on-rocks/scripts/render-product-payload`) + redeploy. The PanVK enablement env (`PAN_I_WANT_A_BROKEN_VULKAN_DRIVER=1`, `MESA_VK_VERSION_OVERRIDE=1.2`, `VK_DRIVER_FILES=...panfrost_icd.aarch64.json`) and the "run gamescope as the primary DRM compositor (LIBSEAT_BACKEND=builtin), not nested in Sway" session-architecture change live in the guest session units → `../nix-on-rocks/guest/modules/rk3566.nix` / sessiond → **fast guest-promote**. No **full image rebuild** is needed (Mesa/PanVK is already on the device); a full rebuild is only required if Mesa itself must be patched/bumped. NOTE: the clean product launch path also depends on task-012 (the sessiond home→game transition).
