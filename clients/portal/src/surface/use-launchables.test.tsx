...
describe("useLaunchables load and core effects", () => {
  test("loads every source and queries apps only for paired stream hosts", async () => {
    ...
    const korrid = buildKorrid(
      {},
      { games: [remoteGame("zao")], localGames: [localGame] },
    )
  ...
  test("keeps the portal usable and skips the bridge when a local ROM is missing", async () => {
    ...
    expect(view.current().state).toMatchObject({
      _tag: "Ready",
      notice: "LocalRomMissing: cannot launch wl4",
    })
  })
  ...
  test("prepares a hosted game and attaches to the stream app on that same host", async () => {
    ...
    const bridge = buildBridge({
      ...
      async startStream(hostUuid, appId) {
        calls.push(`stream:${hostUuid}:${appId}`)
        return { _tag: "StreamStarted" }
      },
    })
    const base = createInMemoryKorridClient({ games: [remoteGame("zao")] })
    const korrid = buildKorrid(
      {
  ...
  test("does not prepare a hosted game when its exact stream host is absent", async () => {
    ...
    await waitFor(
      () =>
        expect(view.current().state).toMatchObject({
          _tag: "Ready",
          notice: 'NoStreamTarget: no "Korri Stream" app on paired host zao',
        }),
  ...
  test("publishes unavailable device action results as notices or settings problems", async () => {
    ...
    await waitFor(
      () =>
        expect(view.current().state).toMatchObject({
          _tag: "Ready",
          notice: "cannot open pairing: no pairing screen",
        }),
  ...
  test("non-timeout status failure after stop returns to ready with a notice", async () => {
    ...
    await waitFor(
      () =>
        expect(view.current().state).toMatchObject({
          _tag: "Ready",
          notice: "HostUnavailable: host offline",
        }),
