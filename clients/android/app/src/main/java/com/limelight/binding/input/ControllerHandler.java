...
public class ControllerHandler implements InputManager.InputDeviceListener, UsbDriverListener {
    ...
    public boolean handleButtonUp(KeyEvent event) {
        ...
        case KeyEvent.KEYCODE_BUTTON_MODE:
            if (prefConfig.enableBackMenu) {
                // Korri: open the overlay on release. When it is already
                // showing, Game's input gate consumes Guide before this
                // handler runs and hides it there — a clean toggle.
        ...
        case KeyEvent.KEYCODE_MENU:
            ...
            if ((context.inputMap & ControllerPacket.PLAY_FLAG) != 0 &&
                ...
                if (prefConfig.enableBackMenu && context.backMenuPending){
                    ...
                    gestures.showGameMenu();
                } else if (prefConfig.mouseEmulation || context.mouseEmulationActive) {
    ...
    public boolean handleButtonDown(KeyEvent event) {
        ...
        case KeyEvent.KEYCODE_BUTTON_MODE:
            context.hasMode = true;
            if (prefConfig.enableBackMenu) {
                // Korri: Guide/Xbox button toggles the in-game overlay.
                // Swallow the press; the overlay opens on release so this
                // press's own up event can't immediately close it. The
                // host never sees it; enable "Back button as Guide" to
                // reach the host's guide button instead.
                return true;
            }
            context.inputMap |= ControllerPacket.SPECIAL_BUTTON_FLAG;
            break;
