...
public class Game extends AppCompatActivity implements SurfaceHolder.Callback,
    ...
    public static final String EXTRA_FRAME_PACING = "FramePacing";
    public static final String EXTRA_ULTRA_LOW_LATENCY = "UltraLowLatency";
    public static final String EXTRA_VIDEO_SCALE_MODE = "VideoScaleMode";
    public static final String EXTRA_CODEC = "Codec";
    public static final String EXTRA_DISPLAY_TOP_CENTER = "DisplayTopCenter";
    public static final String EXTRA_REDUCE_REFRESH_RATE = "ReduceRefreshRate";
    public static final String EXTRA_LOW_LATENCY_FRAME_BALANCE = "LowLatencyFrameBalance";
    public static final String EXTRA_TIGHT_VSYNC = "TightVsync";
    public static final String EXTRA_PIP = "Pip";
    public static final String EXTRA_AUTO_ORIENTATION = "AutoOrientation";
    public static final String EXTRA_FLIP_FACE_BUTTONS = "FlipFaceButtons";
    public static final String EXTRA_HDR = "Hdr";
    public static final String EXTRA_MOUSE_EMULATION = "MouseEmulation";
    public static final String EXTRA_TOUCHSCREEN_MODE = "TouchscreenMode";
    public static final String EXTRA_ABSOLUTE_MOUSE_MODE = "AbsoluteMouseMode";
    ...
    private int overrideTouchscreenModeIndex = -1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Override resolution if both width and height are provided via intent (one-time, not persisted)
        int intentWidth = getIntent().getIntExtra(EXTRA_WIDTH, -1);
        int intentHeight = getIntent().getIntExtra(EXTRA_HEIGHT, -1);
        if (intentWidth > 0 && intentHeight > 0) {
            prefConfig.width = intentWidth;
            prefConfig.height = intentHeight;
        }

        // Override FPS if provided via intent (one-time, not persisted)
        int intentFps = getIntent().getIntExtra(EXTRA_FPS, -1);
        if (intentFps > 0) {
            prefConfig.fps = intentFps;
        }

        // Override bitrate if provided via intent (one-time, not persisted)
        int intentBitrate = getIntent().getIntExtra(EXTRA_BITRATE, -1);
        if (intentBitrate > 0) {
            prefConfig.bitrate = intentBitrate;
        }

        // Override frame pacing if provided via intent (one-time, not persisted)
        String intentFramePacing = getIntent().getStringExtra(EXTRA_FRAME_PACING);
        if (intentFramePacing != null) {
            switch (intentFramePacing) {
                case "latency":
                    prefConfig.framePacing = PreferenceConfiguration.FRAME_PACING_MIN_LATENCY;
                    prefConfig.framePacingWarpFactor = 0;
                    break;
                case "balanced":
                ...
                case "warp":
                    prefConfig.framePacing = PreferenceConfiguration.FRAME_PACING_MAX_SMOOTHNESS;
                    prefConfig.framePacingWarpFactor = 2;
                    break;
                case "warp2":
                    prefConfig.framePacing = PreferenceConfiguration.FRAME_PACING_MAX_SMOOTHNESS;
                    prefConfig.framePacingWarpFactor = 4;
                    break;
                // Invalid values are silently ignored
            }
        }

        // Override ultra low latency if provided via intent (one-time, not persisted)
        String intentUltraLowLatency = getIntent().getStringExtra(EXTRA_ULTRA_LOW_LATENCY);
        if (intentUltraLowLatency != null) {
            prefConfig.enableUltraLowLatency = Boolean.parseBoolean(intentUltraLowLatency);
        }

        // Override video scale mode if provided via intent (one-time, not persisted)
        String intentVideoScaleMode = getIntent().getStringExtra(EXTRA_VIDEO_SCALE_MODE);
        if (intentVideoScaleMode != null) {
            switch (intentVideoScaleMode) {
                case "fit":
                    prefConfig.videoScaleMode = PreferenceConfiguration.ScaleMode.FIT;
                    break;
                case "fill":
        ...
        // Override codec/video format if provided via intent (one-time, not persisted)
        String intentCodec = getIntent().getStringExtra(EXTRA_CODEC);
        if (intentCodec != null) {
            switch (intentCodec) {
                case "auto":
        ...
        // Override display top center if provided via intent (one-time, not persisted)
        String intentDisplayTopCenter = getIntent().getStringExtra(EXTRA_DISPLAY_TOP_CENTER);
        if (intentDisplayTopCenter != null) {
            prefConfig.alignDisplayTopCenter = Boolean.parseBoolean(intentDisplayTopCenter);
        }

        // Override reduce refresh rate if provided via intent (one-time, not persisted)
        String intentReduceRefreshRate = getIntent().getStringExtra(EXTRA_REDUCE_REFRESH_RATE);
        if (intentReduceRefreshRate != null) {
            prefConfig.reduceRefreshRate = Boolean.parseBoolean(intentReduceRefreshRate);
        }

        // Override LFR (low-latency frame balance) if provided via intent (one-time, not persisted)
        String intentLfr = getIntent().getStringExtra(EXTRA_LOW_LATENCY_FRAME_BALANCE);
        if (intentLfr != null) {
            prefConfig.preferLowerDelays = Boolean.parseBoolean(intentLfr);
        }

        // Override tight vsync if provided via intent (one-time, not persisted)
        String intentTightVsync = getIntent().getStringExtra(EXTRA_TIGHT_VSYNC);
        if (intentTightVsync != null) {
            prefConfig.forceTightThresholds = Boolean.parseBoolean(intentTightVsync);
        }

        // Override PIP if provided via intent (one-time, not persisted)
        String intentPip = getIntent().getStringExtra(EXTRA_PIP);
        if (intentPip != null) {
            prefConfig.enablePip = Boolean.parseBoolean(intentPip);
        }

        // Override auto orientation if provided via intent (one-time, not persisted)
        String intentAutoOrientation = getIntent().getStringExtra(EXTRA_AUTO_ORIENTATION);
        if (intentAutoOrientation != null) {
            prefConfig.autoOrientation = Boolean.parseBoolean(intentAutoOrientation);
        }

        // Override flip face buttons if provided via intent (one-time, not persisted)
        String intentFlipFaceButtons = getIntent().getStringExtra(EXTRA_FLIP_FACE_BUTTONS);
        if (intentFlipFaceButtons != null) {
            prefConfig.flipFaceButtons = Boolean.parseBoolean(intentFlipFaceButtons);
        }

        // Override HDR if provided via intent (one-time, not persisted)
        String intentHdr = getIntent().getStringExtra(EXTRA_HDR);
        if (intentHdr != null) {
            prefConfig.enableHdr = Boolean.parseBoolean(intentHdr);
        }

        // Override mouse emulation if provided via intent (one-time, not persisted)
        String intentMouseEmulation = getIntent().getStringExtra(EXTRA_MOUSE_EMULATION);
        if (intentMouseEmulation != null) {
            prefConfig.mouseEmulation = Boolean.parseBoolean(intentMouseEmulation);
        }

        // Override touchscreen mode if provided via intent (one-time, not persisted)
        String intentTouchscreenMode = getIntent().getStringExtra(EXTRA_TOUCHSCREEN_MODE);
        if (intentTouchscreenMode != null) {
            switch (intentTouchscreenMode) {
                case "multitouch": overrideTouchscreenModeIndex = 0; break;
                case "absolute": overrideTouchscreenModeIndex = 1; break;
                case "trackpad-natural": overrideTouchscreenModeIndex = 2; break;
                case "trackpad-gaming": overrideTouchscreenModeIndex = 3; break;
                case "disabled": overrideTouchscreenModeIndex = 4; break;
                case "absolute-swapped": overrideTouchscreenModeIndex = 5; break;
                // Invalid values are silently ignored
            }
        }

        // Override absolute mouse mode if provided via intent (one-time, not persisted)
        String intentAbsoluteMouseMode = getIntent().getStringExtra(EXTRA_ABSOLUTE_MOUSE_MODE);
        if (intentAbsoluteMouseMode != null) {
            prefConfig.absoluteMouseMode = Boolean.parseBoolean(intentAbsoluteMouseMode);
        }
        ...
        // Only the 3D SBS modes force stretch; SGSR (mode 3) preserves the
        // stream aspect ratio like the 2D path.
        if (prefConfig.renderMode == 1 || prefConfig.renderMode == 2) {
            prefConfig.videoScaleMode = PreferenceConfiguration.ScaleMode.STRETCH;
        }
        ...
        // allows proper touch splitting between overlays and stream input.
        ...
        if (prefConfig.enablePerfOverlay) {
            ...
            if (prefConfig.enablePerfOverlayLite) {
                ...
                if(prefConfig.enablePerfOverlayLiteDialog){
                    performanceOverlayLite.setOnClickListener(v -> showGameMenu());
                }
            } else {
...
// Don't stream HDR if the decoder can't support it
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        // Guide toggles the overlay closed. This must live at dispatch
        // level: while the overlay is showing its WebView holds focus, and
        // key events route to Chromium's internal focused child — skipping
        // both the WebView's OnKeyListener (ViewGroup dispatches straight
        // to the focused child) and, for BUTTON_MODE which Chromium
        // consumes, the activity fallback path too.
        if (korriOverlay != null && korriOverlay.isShowing()
    ...
    public boolean handleKeyDown(KeyEvent event) {
        ...
        // Web overlay owns all input while visible; nothing reaches the stream.
        if (korriOverlay != null && korriOverlay.isShowing()) {
            return korriOverlay.handleKeyEvent(event);
        }
    ...
    public boolean handleMotionEvent(View view, MotionEvent event) {
        ...
        // Web overlay owns controller motion while visible (hat/stick -> dpad nav).
        if (korriOverlay != null && korriOverlay.isShowing()) {
            if ((event.getSource() & InputDevice.SOURCE_CLASS_JOYSTICK) != 0) {
                return korriOverlay.handleMotionEvent(event);
            }
            return true;
        }
     ...
     * persisted — the Video Scale Mode setting still owns the default.
     */
    public void toggleFillMode() {
        boolean enableFill =
                prefConfig.videoScaleMode != PreferenceConfiguration.ScaleMode.FILL;
        prefConfig.videoScaleMode = enableFill
                ? PreferenceConfiguration.ScaleMode.FILL
                : PreferenceConfiguration.ScaleMode.FIT;
        streamContainer.setDesiredAspectRatio((double) displayWidth / (double) displayHeight);
        streamContainer.setFillDisplay(enableFill);
        Toast.makeText(this,
                getString(enableFill
                        ? R.string.fill_mode_enabled
                        : R.string.fill_mode_disabled),
    ...
    private void initMouseMode() {
        // Apply override if set via intent
        if (overrideTouchscreenModeIndex != -1) {
            applyMouseMode(overrideTouchscreenModeIndex);
            return;
        }
    ...
    public void selectMouseMode(Context context){
        ...
        for (int i = 0; i < allModes.length; i++) {
            options.add(new MouseModeOption(i, allModes[i]));
        }
    ...
    /**
     * Re-read live-tunable prefs into the in-memory prefConfig. The input,
     * rumble, PiP, and render paths consult prefConfig per-event, so these
     * apply immediately without a stream restart.
     */
    public void applyLivePrefs() {
        android.content.SharedPreferences prefs =
                android.preference.PreferenceManager.getDefaultSharedPreferences(this);
        float sharpness = prefs.getInt("seekbar_sgsr_sharpness", 20) / 10f;
        float threshold = prefs.getInt("seekbar_sgsr_edge_threshold", 8) / 255f;
        prefConfig.sgsrSharpness = sharpness;
        prefConfig.sgsrEdgeThreshold = threshold;
        if (streamContainer != null) {
            streamContainer.updateSgsrParams(sharpness, threshold);
        }
        prefConfig.flipFaceButtons = prefs.getBoolean("checkbox_flip_face_buttons", false);
        prefConfig.enableRumble = prefs.getBoolean("checkbox_enable_rumble", true);
        prefConfig.enablePip = prefs.getBoolean("checkbox_enable_pip", false);
        prefConfig.enableBackMenu = prefs.getBoolean("checkbox_enable_quit_dialog", true);
        prefConfig.enableFloatingButton = prefs.getBoolean("checkbox_enable_floating_button", false);
        prefConfig.mouseEmulation = prefs.getBoolean("checkbox_mouse_emulation", true);
        prefConfig.gamepadTouchpadAsMouse = prefs.getBoolean("checkbox_gamepad_touchpad_as_mouse", false);
        prefConfig.enableDeviceRumble = prefs.getBoolean("checkbox_enable_device_rumble", false);
        prefConfig.backAsGuide = prefs.getBoolean("checkbox_back_as_guide", false);
        prefConfig.backAsMeta = prefs.getBoolean("checkbox_back_as_meta", false);
        prefConfig.disableWarnings = prefs.getBoolean("checkbox_disable_warnings", false);
        updateFloatingButtonVisibility(prefConfig.enableBackMenu && prefConfig.enableFloatingButton);
    }
