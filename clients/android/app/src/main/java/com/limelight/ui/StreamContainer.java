...
public class StreamContainer extends FrameLayout implements SurfaceHolder.Callback, Stereo3DRenderer.OnSurfaceReadyListener {
    ...
    public void init(Game game, PreferenceConfiguration prefConfig) {
        ...
        if (renderMode == StreamMode.MODE_SGSR) {
    ...
    /** Live SGSR tuning from the in-game overlay; no-op in other render modes. */
    public void updateSgsrParams(float sharpness, float threshold) {
        if (mSgsrRenderer != null) {
            mSgsrRenderer.updateEdgeParams(sharpness, threshold);
        }
    }
