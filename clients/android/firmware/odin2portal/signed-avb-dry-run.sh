...
SOURCE="${1:?usage: signed-avb-dry-run.sh <stock-source> <private-key> <output>}"
PRIVATE_KEY="${2:?usage: signed-avb-dry-run.sh <stock-source> <private-key> <output>}"
OUTPUT="${3:?usage: signed-avb-dry-run.sh <stock-source> <private-key> <output>}"
CONTRACT="$HERE/contract"
MARKER="$HERE/marker-build-prop.txt"
NOTICE="$HERE/SIGNED-AVB-NOT-FLASHABLE.md"
...
if [[ ! "$available_bytes" =~ ^[0-9]+$ || "$available_bytes" -lt "$required_free_bytes" ]]; then
  echo 'signed AVB dry run requires at least 32 GiB free in the output filesystem' >&2
  exit 1
fi

required_contract_files=(
  SHA256SUMS active-slot.txt build-fingerprint.txt build-id.txt
  korri-avb-public-key-SHA256.txt logical-SHA256SUMS super-layout.txt
)
...
for file in "$MARKER" "$NOTICE"; do
  [[ -f "$file" && ! -L "$file" ]] || {
    echo "signed AVB input is missing, not regular, or symbolic: $file" >&2
    exit 1
  }
done
...
trap cleanup EXIT
STAGING="$(mktemp -d "$OUTPUT_PARENT/.odin2portal-signed-avb.XXXXXX")"
...
MARKER_OUTPUT="$STAGING/generated-marker-output"
"$HERE/marker-dry-run.sh" "$SOURCE" "$MARKER_OUTPUT" \
  > "$STAGING/evidence/marker-generation.txt"
marker_required=(
...
for file in "${marker_required[@]}"; do
  [[ -f "$MARKER_OUTPUT/$file" && ! -L "$MARKER_OUTPUT/$file" ]] || {
    echo "generated marker output is incomplete or symbolic: $file" >&2
    exit 1
  }
done
grep -Fx 'ODIN2PORTAL_MARKER_DRY_RUN_VERIFIED' "$MARKER_OUTPUT/RESULT.txt" >/dev/null
grep -Fx 'flash ready: no' "$MARKER_OUTPUT/RESULT.txt" >/dev/null
grep -Fx 'device writes: none' "$MARKER_OUTPUT/RESULT.txt" >/dev/null
(
  cd "$MARKER_OUTPUT"
  sha256sum --check evidence/output-SHA256SUMS
) > "$STAGING/evidence/generated-marker-sha256.txt"
...
cp --reflink=auto --sparse=always \
  "$MARKER_OUTPUT/NON_FLASHABLE_ARTIFACTS/product_a.img.not-flashable" "$CUSTOM_PRODUCT"
...
avbtool info_image \
  --image "$MARKER_OUTPUT/NON_FLASHABLE_ARTIFACTS/vbmeta_system_a.img.not-flashable" \
...
(
  ...
  sha256sum \
    NON_FLASHABLE_ARTIFACTS/super.img.not-flashable \
    NON_FLASHABLE_ARTIFACTS/product_a.img.not-flashable \
    > "$STAGING/evidence/generated-marker-artifact-SHA256SUMS"
  sha256sum --check evidence/output-SHA256SUMS
) > "$STAGING/evidence/marker-sha256-final.txt"
mv "$MARKER_OUTPUT/NON_FLASHABLE_ARTIFACTS/super.img.not-flashable" \
...
printf '%s\n' \
  'ODIN2PORTAL_SIGNED_AVB_DRY_RUN_VERIFIED' \
  ...
  'system descriptors: marker-equivalent' \
...
if ! (
  ...
  echo 'source changed during signed AVB dry run' >&2
  exit 1
fi
if ! cmp -s "$CONTRACT/build-id.txt" "$SOURCE/build-id.txt" ||
   ! cmp -s "$CONTRACT/build-fingerprint.txt" "$SOURCE/build-fingerprint.txt" ||
   ! cmp -s "$CONTRACT/active-slot.txt" "$SOURCE/active-slot.txt" ||
   ! cmp -s "$CONTRACT/super-layout.txt" "$SOURCE/super-layout.txt"; then
  echo 'source metadata changed during signed AVB dry run' >&2
  exit 1
fi
[[ "$(sha256sum "$PRIVATE_KEY" | awk '{print $1}')" == "$private_key_sha256" ]] || {
  echo 'private key changed during signed AVB dry run' >&2
  exit 1
}
...
grep -Fx 'ODIN2PORTAL_SIGNED_AVB_DRY_RUN_VERIFIED' "$OUTPUT/RESULT.txt" >/dev/null
...
PUBLISHED_OUTPUT=""
printf 'ODIN2PORTAL_SIGNED_AVB_DRY_RUN_VERIFIED output=%s\n' "$OUTPUT"
