...
> Migrated 2026-06-10 from nix-on-rocks backlog task-029: the bug lives in this
> repo's korri-server; it was originally tracked substrate-side only because it
> blocked the RG353M gamescope story (now task-011 here).

## Why it matters

This is the single thing standing between the RG353M and a non-freezing, product-path gamescope launch. We proved (task-011) that gamescope only renders reliably on Mali-G52 when run as the primary DRM compositor, and that the supported way to get there is the Korri sessiond home→game display transition (which hides the Electrobun webview and hands gamescope the output). That transition is invoked through korri-server's `app.library.launch` RPC — which currently fails for every payload with a Defect, so we were forced into ad-hoc manual `gamescope -- retroarch` launches that hit the nested-compositor freeze. Until this RPC works, there is no clean product launch path on RG353M (and likely none on any device, since it's payload-independent).

## Related

- `product/apps/portal/api/library`
- `korri-server.js launch handler (minified bundle built from this repo)`
- `backlog/task-011 - enable-gamescope-on-rg353m-mali-g52-rk3566.md`
- `../nix-on-rocks/docs/solutions/integration-issues/gamescope-on-mali-g52-panvk-rk3566-2026-06-06.md`
...
- The server is a minified bun bundle: /nix/store/xbj06g3mhhh38wyk6ig210xwnaxmn2gs-korri-server-1.0.0/share/korri-server/korri-server.js. Source is in this repo (product/apps/portal/api/library/*.rpc.ts and the launch handler).
...
1. Find the app.library.launch handler + the LaunchLibraryResponse/managed-launch spec construction; grep for BigIntFromString / BigInt usage in the launch path (launchId, timestamps, playtime/stats).
...
DEPLOY / REBUILD LANE: **Payload re-render.** The fix is in this repo (korri-server bun bundle). It reaches the device by rebuilding the product payload and `../nix-on-rocks/scripts/render-product-payload` + redeploying the payload to the guest — **fast**, no SD reflash and no guest-system rebuild. A **full image rebuild** is only needed if a fresh seeded baseline is wanted. Pairs with task-011's payload re-render (same lane), so both can ship together.
